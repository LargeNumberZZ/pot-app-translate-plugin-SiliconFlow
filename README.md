...
- **智能模式（默认）**：自动识别待翻译内容——
  - **单词 / 短语 / 固定搭配**（如 `hello`、`break down`、`画蛇添足`）：调用 Qwen2.5-7B 输出结构化词典，以 **pot 原生词典卡片**展示（与内置 ecdict / 必应词典效果一致）：音标（英语词条同时给美式 us 和英式 uk 的 IPA；日文给假名；中文给拼音）、分词性释义、复数 / 过去式等屈折变化、例句对照；
...
> - 默认的"词典模式 Prompt"要求模型输出 pot 词典 JSON 结构以获得词典卡片效果；若你自定义该 Prompt，输出将按纯文本展示（解析不出词典 JSON 时，插件会自动用内置的纯文本词典格式重试一次）。
