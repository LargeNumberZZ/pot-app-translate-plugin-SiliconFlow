...
- **智能模式（默认）**：自动识别待翻译内容——
  - **单词 / 短语 / 固定搭配**（如 `hello`、`break down`、`画蛇添足`）：**千问 JSON 词典优先**（词性标注准确），**混元快速译文并行先行展示**（1~2 秒）；千问失败时混元行格式词典兜底。卡片为极简版：只有英美音标 + 按词性分组的词义；
  - **短句 / 长句 / 段落**：**混元优先**（翻译特调、意译自然），千问兜底。只翻译不润色，不使用 markdown，分段换行与原文完全一致。
- **双模型对照**：同时调用两个模型；句子两栏**实时流式**显示；词典与智能模式同为千问优先、混元兜底；单个模型失败不影响另一个。
- **仅混元 / 仅 Qwen**：快速译文、词典、句子翻译均由同一个模型完成（同模型请求串行发出，避免免费档并发限制）。
- **按需详细解释**：默认词典卡片是极简版（只有英美音标、按词性分组的词义，混元生成、速度快）；卡片底部有"详细解释"链接，点击后原地加载详尽版——**详尽版由千问生成**（每词性 2~4 条释义含引申义、3~5 条搭配、屈折变化、近义词、用法说明、两组例句），千问失败时自动改用混元。详细请求会把简明版内容一并带给模型要求保留并扩充，插件侧还会做**合并兜底**，详尽版永远包含简明版的全部内容。详尽版带"◂ 返回简明版"和"↻ 刷新"（强制重新获取），简明/详细两版都有缓存，切换不发请求。
...
- **显示策略**：优先走 webview 原生 fetch 真流式（文字边生成边出现，硅基流动 CORS 开放）；不可用时自动回退 tauriFetch 并以约 0.4 秒的快速打字动画展示。**请求带 20 秒超时**：超时若已有部分生成内容则直接截断返回（真流式下通常已有大半结果），完全没有内容才报超时错误。词典输出经过**去重加固**（重复的音标/词性/联想/例句只保留一条），模型复读不会撑爆卡片；解析失败时自动降级为纯文本展示。

> "详细解释"按钮利用 pot 渲染词典卡片例句字段时允许内联 HTML 的特性实现（当前 pot 版本验证可用）；若未来 pot 上游更改渲染方式导致按钮失效，仅该按钮不可用，卡片内容不受影响。
> 模型分工小结：**句子翻译 = 混元优先（意译自然）、千问兜底**；**词典（简明 + 详细）= 千问优先（词性准确）、混元兜底**；快速译文 = 混元。单模型模式（仅混元/仅 Qwen）下所有环节都只用所选模型。词典请求温度 0.1，尽量稳定输出格式。

> 关于速度：免费档 7B 模型的生成速度有限（约 30~50 token/s），词典卡片与长段落翻译需要生成大量 token，完整输出通常需要数秒到十几秒——这是模型输出速度的硬限制。插件通过"快速译文先行 + 词典卡片后到"、真流式显示、压缩输出要求、词典请求低温（0.1）等方式尽量缩短可感知的等待。
