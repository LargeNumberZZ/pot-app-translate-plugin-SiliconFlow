...
- **智能模式（默认）**：自动识别待翻译内容——
  - **单词 / 短语 / 固定搭配**（如 `hello`、`break down`、`画蛇添足`）：**混元快速译文先行（1~2 秒）**，随后混元按严格的行格式输出词典内容，插件解析成 **pot 原生词典卡片**（实测混元的词典内容质量比 Qwen 更好：音标、每词性 1~3 条释义、屈折变化、搭配、例句）；Qwen 的 JSON 词典并行生成作为兜底；
  - **短句 / 长句 / 段落**：调用 Hunyuan-MT-7B 直接给出流畅译文。
- **双模型对照**：同时调用两个模型；句子两栏**实时流式**显示，词典以混元解析卡片优先、Qwen 词典卡片兜底；单个模型失败不影响另一个。
- **仅混元 / 仅 Qwen**：快速译文、词典、句子翻译均由同一个模型完成（同模型请求串行发出，避免免费档并发限制）。混元词典通过行格式文本解析同样能出词典卡片。
- **按需详细解释**：默认词典卡片是简明版（速度快，混元生成）；卡片底部有"详细解释"链接，点击后原地加载详尽版——**详尽版由千问生成**（每词性 2~4 条释义、3~5 条搭配、近义词、用法说明、两组例句，JSON 格式是千问强项），千问失败时自动改用混元；详尽版带"◂ 返回简明版"和"↻ 刷新"（强制重新获取），简明/详细两版都有缓存，切换不发请求。
...
- **显示策略**：优先走 webview 原生 fetch 真流式（文字边生成边出现，硅基流动 CORS 开放）；不可用时自动回退 tauriFetch 并以约 0.4 秒的快速打字动画展示。请求带 90 秒超时保护，模型空转不会无限转圈。词典输出经过**去重加固**（重复的音标/词性/联想/例句只保留一条），模型复读不会撑爆卡片；解析失败时自动降级为纯文本展示。

> "详细解释"按钮利用 pot 渲染词典卡片例句字段时允许内联 HTML 的特性实现（当前 pot 版本验证可用）；若未来 pot 上游更改渲染方式导致按钮失效，仅该按钮不可用，卡片内容不受影响。
> 模型分工小结：**简明卡片 = 混元（快）**；**详细解释 = 千问（质量好，混元兜底）**；句子翻译 = 混元（翻译特调）。单模型模式（仅混元/仅 Qwen）下所有环节都只用所选模型。

> 关于速度：免费档 7B 模型的生成速度有限（约 30~50 token/s），词典卡片与长段落翻译需要生成大量 token，完整输出通常需要数秒到十几秒——这是模型输出速度的硬限制。插件通过"快速译文先行 + 词典卡片后到"、真流式显示、压缩输出要求、词典请求低温（0.3）等方式尽量缩短可感知的等待。
