...
- **智能模式（默认）**：自动识别待翻译内容——
  - **单词 / 短语 / 固定搭配**（如 `hello`、`break down`、`画蛇添足`）：**并行请求两个模型**——混元几秒内先给出快速译文，Qwen2.5-7B 输出结构化词典，就绪后以 **pot 原生词典卡片**自动替换（与内置 ecdict / 必应词典效果一致）：音标（英语词条同时给美式 us 和英式 uk 的 IPA；日文给假名；中文给拼音）、分词性释义（每词性 1~3 条）、复数 / 过去式等屈折变化、例句对照；
  - **短句 / 长句 / 段落**：调用 Hunyuan-MT-7B 直接给出流畅译文。
- **双模型对照**：同时调用两个模型；句子两栏**实时流式**显示，词典结果**任一先就绪即显示**；单个模型失败不影响另一个。
- **全部由混元 / 全部由 Qwen 输出**：快速译文、词典、句子翻译均由同一个模型完成（同模型的多个请求串行发出，避免免费档并发限制）。注意：混元是纯翻译特调模型，JSON 指令遵循很差，因此该模式下词典自动改用**纯文本词典格式**输出（不再尝试词典卡片）。
...
- **显示策略**：优先走 webview 原生 fetch 真流式（文字边生成边出现，硅基流动 CORS 开放）；不可用时自动回退 tauriFetch 并以约 0.4 秒的快速打字动画展示。请求带 90 秒超时保护，模型空转不会无限转圈。词典 JSON 解析失败（含字段全空）时自动改用纯文本词典格式重试一次，不会把原始 JSON 展示给用户。
